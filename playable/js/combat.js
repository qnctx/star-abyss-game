(function (SA) {
  ...
  function kill(s, enemy, cause) { var eligible = cause === 'player' || cause === 'turret'; if (eligible && !enemy.rewarded) { enemy.rewarded = true; R.add(s.inventory, C.enemy.reward); s.kills += 1; s.score += 50; } enemy.dead = true; }
  function tick(s, dt) {
    s.buildings.filter(function (b) { return b.type === 'turret' && b.hp > 0; }).forEach(function (t) { t.timer -= dt; t.fireFlash = Math.max(0, (t.fireFlash || 0) - dt); var target = nearestEnemy(s, t, C.buildings.turret.range + t.level * C.runtime.turretRangePerLevel); if (target && t.timer <= 0) { t.aimAngle = Math.atan2(target.y - t.y, target.x - t.x); t.fireFlash = C.runtime.turretFireFlashDuration; damageEnemy(s, target, C.runtime.turretDamage + t.level * C.runtime.turretDamagePerLevel, 'turret'); t.timer = Math.max(C.runtime.turretCooldownMin, C.runtime.turretCooldown - t.level * C.runtime.turretCooldownLevelReduction); } });
    ...
    s.enemies.forEach(function (e) {
      ...
      if (R.distance(e, target) < (target === s.base ? C.base.radius : 24) && e.attack <= 0) { e.attack = C.runtime.enemyAttackCooldown; if (target === s.player) { if (s.player.invulnerable <= 0) { s.player.hp -= C.enemy.contactPlayer; s.player.invulnerable = C.runtime.playerContactInvulnerability; if (s.player.hp <= 0) R.die(s); } } else if (target === s.base) { damageBase(s, C.enemy.contactBase); kill(s, e, 'impact'); } else { target.hp -= C.runtime.structureContactDamage; kill(s, e, 'impact'); } }
    });
